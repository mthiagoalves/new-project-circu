...
class Product3dJob implements ShouldQueue
{
    ...
    // });
    Mail::send('mail.auto-reply.product-3d', ['request' => $request, 'product' => $product, 'product_name' => $product_name], function($message) use ($request) {
        $message->from('info@circu.net', 'Circu Magical Furniture');
        $message->to($request['email'])->subject('Circu | Product 3D');
    });
      }
}