...
<style type="text/css">
	...
	.background-prev{
		...
		background: url('/img/inspirations/thumbnails/{{$prevInspiration->slug}}/{{$prevInspiration->slug}}-thumbnail.jpg')
	}
	...
	.background-next{
		...
		background-position: center !important;
		background-size: cover !important;
		background: url('/img/inspirations/thumbnails/{{$nextInspiration->slug}}/{{$nextInspiration->slug}}-thumbnail.jpg')
	}
...
<div class="container-fluid p-0">
	<div class="row m-0 justify-content-between">
		<div class="swiper-container swiper-ambience-inspirations">
			<div class="swiper-wrapper my-gallery">
				<div class="swiper-slide background-next" style="max-width:600px;">
					<a href="/inspirations/{{$prevInspiration->slug}}" class="overlay-background">
						<div class="col-8 content-overlay">
							<h2 class="title-overlay mb-4">{{$prevInspiration->projects_languages[0]->title}}</h2>
				...
				@php
				$directory = $_SERVER['DOCUMENT_ROOT'].'/img/inspirations/banners/' . $project->slug . '/';
				...
				@for ($i = 1; $i <= $filecount; $i++)
					<div class="swiper-slide slider-main find-them-all" style="margin-bottom: 0;">
						<img draggable="false" alt="{{$project->projects_languages[0]->title}}" src="/img/inspirations/banners/{{$project->slug}}/{{$project->slug}}-banner-{{$i}}.jpg" alt="Image description" style="cursor: pointer;width:100%;" class="img-fluid d-none d-md-block pswp--open-r imgs-hotspots" />
					</div>
				@endfor
				<div class="swiper-slide background-next" style="max-width:600px;">
					<a href="/inspirations/{{$nextInspiration->slug}}" class="overlay-background">
						<div class="col-8 content-overlay">
							<h2 class="title-overlay mb-4">{{$nextInspiration->projects_languages[0]->title}}</h2>
							<i class="fa fa-eye" aria-hidden="true"></i>
							<p>KNOW MORE</p>
						</div>
					</a>
