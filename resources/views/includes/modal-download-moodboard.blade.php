...
<body>
    ...
    <form method="post" class="form-popup form-parsley" action="{{ route('process.download-moodboard', ['moodboard' => $moodboard[0]->slug] ) }}">

        <div class="row">

            <div class="form-popup-header col-12">
            <h1 >DOWNLOAD MOODBOARD</h1>
            <h3 class="pb-4">{{$moodboard[0]->article_languages[0]->title}}</h3>
            </div>

            {{ csrf_field() }}

            
            <input type="hidden" name="product_name" value="{{$moodboard[0]->slug}}" />
            ...
            <div class="col-12">
                <button type="submit" onclick="ga('send','event','Get Price','Request Price','Product-getprice');" id="request-price-tag" class="btn-request-piece btn-prevent-multiple"><i class="spinner-visible fas fa-spinner fas-spin"></i> DOWNLOAD NOW</button>
            </div>
