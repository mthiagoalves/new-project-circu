...
<body>
    ...
    <form method="post" class="form-popup form-parsley" action="{{ route('process.download-ebooks-catalogues', ['ebook' => $ebook->slug] ) }}">

          <div class="row">
            
            <div class="form-popup-header col-12">
                <h1 class="text-uppercase"> DOWNLOAD {{$ebook->key_words}}</h1> 
                <h3>{{ $ebook->article_languages[0]->title }}</h3>
            </div>
            ...
            <div class="col-12">
                <button type="submit" onclick="ga('send','event','Get Price','Request Price','Product-getprice');" id="request-price-tag" class="btn-request-piece btn-prevent-multiple"><i class="spinner-visible fas fa-spinner fas-spin"></i>DOWNLOAD NOW <i class="fas fa-caret-right"></i></button>
            </div>
